...
- (void)viewDidAppear:(BOOL)animated {
    ...
    SD_DELAY_HANDLER(0.50f, {
        ...
        @" \
【显示与隐藏】 \n \
    · 当App启动后，通过`摇一摇`手势启动调试器(如果是Xcode模拟器，可以使用快捷键command+control+z)，默认会出现在屏幕右下角，再次`摇一摇`会隐藏调试器，通过代码传入一个手势类实例可以更改成其他触发方式。 \n\n \
【功能集】 \n \
    · 目前研发完整的功能有 离散API日志收集、绑定API日志收集、系统日志收集、崩溃拦截捕获、简易APM、野指针探测器、内存泄漏检测器。 \n \
    · 目前尚在研发的功能有 循环引用探测。 \n \
    · 未来会设计出一些更加强大的功能，敬请期待。 \n\n \
【基本操作】 \n \
    · 通过`摇一摇`手势或`自定义手势`呼出调试器后，快速单点灯泡图标进入`工具集页面` \n \
...
【写给研发】 \n \
    ...
    · 当程序发生Crash后，调试器能在大部分情况下捕获抛出的异常(包括BSD层发出signal)，并在Crash现场立即弹出友好的`崩溃信息详情页面`以供开发第一时间发现导致崩溃的代码，但是由于崩溃后程序已处于不稳定状态，如果崩溃代码发生在多线程甚至更复杂的环境下，`崩溃信息详情页面`可能会弹出失败(也可能会出现假死的状态)，此时可以直接手动kill掉App，重启后通过点击工具集中的`Crash Captor`查看已被捕获的崩溃详情(如果是处于Xcode连接状态的真机或者模拟器也可以在Xcode控制台直接查看本工具辅助打印的崩溃堆栈信息)，当然工具也提供所谓的`安全模式`，设置为安全模式后，捕获被捕获后会推迟到App重启后提示开发者。 \n \
    · API日志收集在模拟器中通过快捷键在业务页面与调试器页面之间切换翻阅效率要高于Xcode与模拟器之间切换 \n \
    · 可以定制调试器的触发手势，通过TDFSDManager的`- (void)registerQuickLaunchGesture:(__kindof UIGestureRecognizer *)launchGesture forSubTool:(SDSubToolType)subTool;`方法在程序启动后通过代码传入一个UIGestureRecognizer的子类调用生效。 \n\n \
【写给测试】 \n \
